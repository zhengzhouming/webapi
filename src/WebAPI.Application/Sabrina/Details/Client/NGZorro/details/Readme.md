...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.DetailsServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'Details',i18n:'Details',acl:'Pages.Details',icon:'iconfont icon-dashboard',link:'/app/main/details',sort:99},

```

请注意：以上link的值 '/app/main/details') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('Details', 'Pages.Details', 'iconfont icon-dashboard', '/app/main/details')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'details', component:DetailsComponent,data: { permission: 'Pages.Details' }  }, 
