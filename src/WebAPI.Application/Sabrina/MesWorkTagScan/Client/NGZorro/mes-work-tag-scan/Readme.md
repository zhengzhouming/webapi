...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.MesWorkTagScanServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'MesWorkTagScan',i18n:'MesWorkTagScan',acl:'Pages.MesWorkTagScan',icon:'iconfont icon-dashboard',link:'/app/main/mesworktagscan',sort:99},

```

请注意：以上link的值 '/app/main/mesworktagscan') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('MesWorkTagScan', 'Pages.MesWorkTagScan', 'iconfont icon-dashboard', '/app/main/mesworktagscan')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'mesworktagscan', component:MesWorkTagScanComponent,data: { permission: 'Pages.MesWorkTagScan' }  }, 

```
或
//不带权限验证
```
{  path: 'mesworktagscan', component:MesWorkTagScanComponent }, 
```
...
```
MesWorkTagScanComponent,
CreateOrEditMesWorkTagScanComponent,
