...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.MesUserServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'MesUser',i18n:'MesUser',acl:'Pages.MesUser',icon:'iconfont icon-dashboard',link:'/app/main/users',sort:99},

```

请注意：以上link的值 '/app/main/users') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('MesUser', 'Pages.MesUser', 'iconfont icon-dashboard', '/app/main/users')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'users', component:MesUserComponent,data: { permission: 'Pages.MesUser' }  }, 

```
或
//不带权限验证
```
{  path: 'users', component:MesUserComponent }, 
```
