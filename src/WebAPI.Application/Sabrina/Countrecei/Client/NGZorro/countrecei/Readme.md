...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.CountReceiServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'CountRecei',i18n:'CountRecei',acl:'Pages.CountRecei',icon:'iconfont icon-dashboard',link:'/app/main/countrecei',sort:99},

```

请注意：以上link的值 '/app/main/countrecei') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('CountRecei', 'Pages.CountRecei', 'iconfont icon-dashboard', '/app/main/countrecei')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'countrecei', component:CountReceiComponent,data: { permission: 'Pages.CountRecei' }  }, 

```
或
//不带权限验证
```
{  path: 'countrecei', component:CountReceiComponent }, 
```
...
```
CountReceiComponent,
CreateOrEditCountReceiComponent,
