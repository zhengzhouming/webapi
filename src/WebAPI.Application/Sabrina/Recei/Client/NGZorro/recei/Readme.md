...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.ReceiServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'Recei',i18n:'Recei',acl:'Pages.Recei',icon:'iconfont icon-dashboard',link:'/app/main/recei',sort:99},

```

请注意：以上link的值 '/app/main/recei') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('Recei', 'Pages.Recei', 'iconfont icon-dashboard', '/app/main/recei')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'recei', component:ReceiComponent,data: { permission: 'Pages.Recei' }  }, 

```
或
//不带权限验证
```
{  path: 'recei', component:ReceiComponent }, 
```
