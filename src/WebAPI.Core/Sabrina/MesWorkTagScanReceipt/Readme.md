...
- 前端代码生成的文件文件路径请在WebAPI.Application类库中生成的对应实体代码文件中的Client中。
...
如果你选择了**生成权限功能**，请打开WebAPI.Application类库中的WebAPIApplicationModule.cs类文件。
...
请打开WebAPI.Application类库中WebAPIApplicationModule.cs中的 PreInitialize 方法中:
...
打开EntityFrameworkCore类库在 **WebAPIDbContext**类文件中添加以下代码段：
...
{
   modelBuilder.ApplyConfiguration(new MesWorkTagScanReceiptCfg());
 }
...
```
Add-Migration AddNewMesWorkTagScanReceiptEntity_Migration
```
...
生成的多语言内容
在WebAPI.Core类库中对应实体下的`douyuyan.md`文件中。
